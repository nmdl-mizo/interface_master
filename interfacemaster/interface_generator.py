...
def write_POSCAR(lattice, atoms, elements, filename='POSCAR'):
    ...
    with open(filename, 'w', encoding="utf-8") as f:
        ...
        f.write("1\n")
        for ai in lattice.T:
            f.write(' '.join(list(map(lambda x: f'{x:.16f}', ai))).rstrip() + '\n')

        # elements
        f.write(' '.join(element_species).rstrip() + '\n')

        # num of atoms
        f.write(' '.join(map(str, num_list)).rstrip() + '\n')
...
class core:
    ...
    def get_bicrystal(self, dydz=None, dx=0, dp1=0, dp2=0,
        ...
        if filetype == 'VASP':
            # Option 1: Write the POSCAR file using a custom function (choose this OR the next option).
            write_POSCAR(lattice_bi, atoms_bi, elements_bi, filename)
            # OR
            # Option 2: Use pymatgen's Poscar class to write the file (alternative to the above).

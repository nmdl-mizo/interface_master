...
def write_LAMMPS(lattice, atoms, elements, filename = 'lmp_atoms_file', orthogonal = False):
    ...
    with open(filename, 'w') as f:
        ...
        for i in range(len(element_indices)):
            f.write(f'{element_indices[i]} {element_species[i]} ')
        f.write(f'\n {len(atoms)} atoms \n \n')
        ...
        for i in range(n1):
            for j in range(n2):
                ...
                self.get_bicrystal(dydz = dydz, dx = dx, dp1 = dp1, dp2 = dp2, \
                      xyz_1 = xyz_1, xyz_2 = xyz_2, vx = vx, two_D = two_D, filename = f'CNID_inputs/{filename}_{i}_{j}', filetype = filetype)
        if self.verbose:
            print('completed')
    ...
    def define_lammps_regions(self, region_names, region_los, region_his, ortho = False):
        ...
        with open('blockfile', 'w') as fb:
            for i in range(len(region_names)):
                if not ortho:
                    fb.write(f'region {region_names[i]} prism {region_los[i]} {region_his[i]} EDGE EDGE EDGE EDGE {xy} {xz} {yz} units box \n')
                else:
                    fb.write(f'region {region_names[i]} block {region_los[i]} {region_his[i]} EDGE EDGE EDGE EDGE units box \n')
                fb.write(f'group {region_names[i]} region {region_names[i]} \n')
...
def get_surface_slab(structure, hkl, replica = [1,1,1], inclination_tol = sqrt(2)/2, termi_shift = 0, vacuum_height = 0, plane_normal = False, normal_perp = False, \
    ...
    inclination_tol : float
        required minimum cos value of the angle between the basic crossing vector and the surface, default sqrt(2)/2
    termi_shift : float
